...
class Client(val solr: no.met.solr.SClient) {
  ...
  def result( res: SolrDocumentList ): Seq[Location] = {
    val places = for (p <- res.asScala) yield {
      for{
        ...
        geometry <- Try(p.getFieldValue("geometry").asInstanceOf[String]);
        p=Location( ssrId, ssrObjId, nameType, location, language, geometry)
      } yield (p)
    }
  ...
  def query(lon: Double, lat: Double, dist: Int, limit: Option[Int]): Try[Seq[Location]] = Try {
    val q = new SolrQuery("*:*")
      .addFilterQuery(s"{!geofilt}&sfield=location&pt=${lon} ${lat}&d=${dist}")
      .addSort("geodist()", SolrQuery.ORDER.asc)
      .setFields("ssrId,ssrObjId,name_type,location,language,geometry", "_dist_:geodist()")
  ...
  def query(namePattern: String, limit: Option[Int]): Try[Seq[Location]] = Try{
    val q = new SolrQuery(s"location_search:$namePattern")
      .addSort("location", SolrQuery.ORDER.asc)
      .setFields("ssrId,ssrObjId,name_type,location,language,geometry")
